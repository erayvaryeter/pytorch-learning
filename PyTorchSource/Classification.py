...
class TransferLearningModelTrainer:
    def __init__(self, data_dir):
        ...
        self.image_datasets = {x: datasets.ImageFolder(os.path.join(data_dir, x), self.data_transforms[x])
                                for x in ['train', 'val']}
        self.dataloaders = {x: torch.utils.data.DataLoader(self.image_datasets[x], batch_size=16, shuffle=True, num_workers=4)
                                for x in ['train', 'val']}
...
def main():
    ...
    trainer = TransferLearningModelTrainer('D:/Datasets/Age')
    trainer.train()
